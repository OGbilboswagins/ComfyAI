...
const ModelDownloadModal: React.FC<IProps> = (props) => {
  ...
  const getModelSuggests = async (value: string) => {
    setLoading(true)
    try {
      const response = await fetch(`/api/model-suggests?keyword=${value}`)
