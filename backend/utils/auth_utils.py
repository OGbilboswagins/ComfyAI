...
def extract_and_store_api_key(request) -> Optional[str]:
    ...
    try:
        ...
        if auth_header and auth_header.startswith('Bearer '):
            ...
            print(f"ComfyUI Copilot API key extracted and stored: {api_key[:12]}...")
            
            # Verify it's stored correctly
            stored_key = get_comfyui_copilot_api_key()
            if stored_key == api_key:
                print("API key verification: ✓ Successfully stored in globals")
            else:
                print("API key verification: ✗ Storage failed")
        ...
        print(f"Error extracting API key: {str(e)}")
        return None
