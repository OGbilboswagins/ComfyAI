...
async def analyze_missing_connections() -> str:
    ...
    返回格式说明：
    - missing_connections: 缺失连接的详细列表，包含节点ID、输入名称、需要的数据类型等（仅包含required且非universal的输入）
      每个missing_connection包含connectable_outputs字段，列出所有可连接的输出端口信息
    - universal_inputs: 类型为*的required输入端口，可以接受任意输出类型
    ...
    try:
        ...
        for node_id, node_data in workflow_data.items():
            ...
            for input_name, input_config in required_inputs.items():
                if input_name not in current_inputs:
                    ...
                    # 检查是否是通用输入（input类型为*）
                    is_universal_input = expected_types == ["*"]
                    ...
                    if is_universal_input:
                        ...
                        for source_node_id, source_info in available_outputs.items():
                            ...
                            for output_index, output_type in source_info["outputs"]:
                                # 检查类型匹配：expected_types相等 或 output类型为*
                        ...
                        # 添加匹配的输出信息到missing_connection
                        missing_connection["connectable_outputs"] = matching_outputs
                        ...
                        # 只有非universal的input才加入missing_connections
        ...
        log.info(f"analysis_link_result: {json.dumps(analysis_result, ensure_ascii=False)}")
